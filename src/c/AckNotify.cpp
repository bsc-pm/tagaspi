		...
		const gaspi_queue_id_t queue)
{
	...
	gaspi_return_t eret = gaspi_operation_get_num_requests(GASPI_OP_NOTIFY, 0, &numRequests);
	...
	if (remaining == 0) {
		...
		eret = gaspi_operation_submit(GASPI_OP_NOTIFY,
			tag, 0, 0, rank, segment_id_remote, 0, 0,
